...
impl ArtifactClient for RedisArtifactClient {
    ...
    async fn add_artifact_user(&self, artifact_id: &str, task_id: &str) -> Result<bool> {
        let mut conn = self
            .get_redis_connection(artifact_id)
            .await
            .map_err(|e| anyhow!(e))?;
        
        let key = format!("artifact_users:{}", artifact_id);
        
        // Add task to the artifact's user set
        let added: bool = conn.sadd(&key, task_id).await?;
        
        // Set expiration
        conn.expire(&key, ARTIFACT_TIMEOUT_SECONDS as i64).await?;
    ...
    async fn try_delete_when_unused(&self, artifact: &impl ArtifactId, task_id: &str, artifact_type: ArtifactType) -> Result<bool> {
        ...
        let artifact_key = format!("artifact_users:{}", artifact_id);
        
        // Remove task from the artifact's user set
        let _removed: bool = conn.srem(&artifact_key, task_id).await?;
        
        // Check if any users remain
        let remaining_count: i64 = conn.scard(&artifact_key).await?;
        
        if remaining_count == 0 {
            // Clean up the empty set
            conn.del(&artifact_key).await?;
            
            // Last reference - safe to delete artifact
    ...
    async fn cleanup_task_artifacts(&self, task_id: &str) -> Result<Vec<String>> {
        // This method is not needed for this simple implementation
        // since we don't track bidirectional relationships
        Ok(vec![])
    }
}
